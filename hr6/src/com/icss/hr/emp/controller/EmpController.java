...
public class EmpController {
	...
	@ResponseBody
	public List<Emp> queryByIndex(String queryStr,HttpServletRequest request,HttpServletResponse response) throws ParseException, IOException, InvalidTokenOffsetsException {
	...
	public void sendEmail(String to,String title,String content,HttpServletRequest request,HttpServletResponse response) throws ParseException, IOException, InvalidTokenOffsetsException {
		
		//根据用户名获得发送方的邮箱地址，用户名和密码
		HttpSession session = request.getSession();
		String empLoginName = (String) session.getAttribute("empLoginName");
		
		Emp emp = service.queryByLoginName(empLoginName);
		
		MailUtil.sendMail(emp.getEmpEmail(), emp.getEmpEmail(), "123456", to, title, content);
